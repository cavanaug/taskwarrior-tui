...
impl KeyConfig {
  ...
  pub fn update(&mut self, data: &str) -> Result<()> {
    ...
    let redraw = Self::get_config("uda.taskwarrior-tui.keyconfig.redraw", data);
    ...
    self.redraw = redraw.unwrap_or(self.redraw);
...
mod tests {
  ...
  fn test_priority_keys_configurable() {
    // Use keys that don't collide with any defaults (shortcut1-4 default to '1'-'4')
    let data = "uda.taskwarrior-tui.keyconfig.priority-h <F5>\n\
                uda.taskwarrior-tui.keyconfig.priority-m <F6>\n\
                uda.taskwarrior-tui.keyconfig.priority-l <F7>\n\
                uda.taskwarrior-tui.keyconfig.priority-n <F8>\n";
    let kc = KeyConfig::new(data).unwrap();
    assert_eq!(kc.priority_h, KeyCode::F(5));
    assert_eq!(kc.priority_m, KeyCode::F(6));
    assert_eq!(kc.priority_l, KeyCode::F(7));
    assert_eq!(kc.priority_n, KeyCode::F(8));
  }
